...
if args.full_training:
    output_file = f'out_pretrain/lessdata/{args.data_name}/{args.data_name}-full_{args.seed}'
else:
    output_file = f'out_pretrain/lessdata/{args.data_name}/{args.data_name}_{args.seed}'
...
pretrain_fraction = args.partial
if pretrain_fraction and pretrain_fraction != 1.0:
    num_samples = int(len(X_train_intp) * pretrain_fraction)

    indices = torch.randperm(len(X_train_intp))[:num_samples]

    X_train_intp = X_train_intp[indices]
...
output_folder = 'out_pretrain/'
if args.extension:
    pretrain_folder = f'model_pretrain/{args.extension}'
    output_folder = f'out_pretrain/{args.extension}'

os.makedirs(f'{output_folder}', exist_ok=True)
os.makedirs(f'{pretrain_folder}/{args.data_name}', exist_ok=True)
os.cpu_count()
...
scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(encoder_optimizer, mode='min', factor=0.1, patience=5)


loss_list = []
best_valid_loss = float('inf')
best_model_path = f'{pretrain_folder}/{args.data_name}/{args.data_name}_{args.seed}.pth'
output_file = f'{output_folder}/{args.data_name}/{args.data_name}_{args.seed}'
