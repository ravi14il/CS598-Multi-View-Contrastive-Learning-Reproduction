...
k = 0
args = parse_args()
seed_everything(args.seed+15+k)

pretrain_folder =  f'model_pretrain/'
output_folder = 'out_finetune/'
if args.extension:
    pretrain_folder = f'model_pretrain/{args.extension}'
    output_folder = f'out_finetune/{args.extension}'
...
target_name  = args.data_name
source_name  = getattr(args, "pretrain_data_name", None) or target_name
#
...
## Load data
with open(f'preprocessed_data/{target_name}.pkl', 'rb') as f:
...
os.makedirs(f'{output_folder}/{args.data_name}', exist_ok=True)
...
## Run -- finetune
best_model_path = f'{pretrain_folder}/{source_name}/{source_name}_{args.seed}.pth'
...
encoder_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(encoder_optimizer, mode='max', factor=0.1, patience=10)
clf_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(clf_optimizer, mode='max', factor=0.1, patience=10)

loss_list = []
metric_list = []
best_valid_mm = 0
best_valid_loss = float('inf')
output_file = f'{output_folder}/{args.data_name}-{source_name}/{args.data_name}_{args.seed}_{args.feature}_{args.loss_type}_{args.lam}_{k}_finetune'
...
if os.path.exists(output_file):
    ...
    for epoch in range(1, args.epochs_finetune + 1):
        ...
        enc_lrs = [g['lr'] for g in encoder_optimizer.param_groups]
        clf_lrs = [g['lr'] for g in clf_optimizer.param_groups]
        print(f"\nEpoch {epoch}: encoder LRs = {enc_lrs}, classifier LRs = {clf_lrs}")
        ...
        print(
            f"Epoch {epoch} [TRAIN] acc={train_metric['accuracy']:.4f}, ",
            str(train_metric['f1_score'])
        )
        print(
            f"Epoch {epoch} [VALID] acc={valid_metric['accuracy']:.4f}, ",
            str(valid_metric['f1_score'])
        )
        print(
            f"Epoch {epoch} [TEST]  acc={test_metric['accuracy']:.4f}, ",
            str(test_metric['f1_score'])
        )
...
best_valid_mm = 0
best_valid_loss = float('inf')
output_file = f'{output_folder}/{args.data_name}-{source_name}/{args.data_name}_{args.seed}_{args.feature}_{args.loss_type}_{args.lam}_{k}_freeze'
...
output_file = f'{output_folder}/{args.data_name}/{args.data_name}_{args.seed}_{args.feature}_{args.loss_type}_{args.lam}_{k}_baseline'
